...
context.set_context(mode=context.GRAPH_MODE, enable_auto_mixed_precision=True, save_graphs=False,
                    device_target="Ascend", device_id=int(os.getenv('DEVICE_ID'))   #设置GPU,选择Ascend
...
def parse_args():
    ...
    parser.add_argument('--data_file', type=str, default='',
                        help='path and name of one mindrecord file')
    ...
    # optimizer
    parser.add_argument('--train_epochs', type=int, default=20, help='epoch')
    ...
    parser.add_argument('--rank', type=int, default=1, help='local rank of distributed')
...
def train():
    ...
    #自定义数据集，定义加载器
    dataset_generator = DatasetGenerator(data_root="/cache/gis")
    dataset = ds.GeneratorDataset(dataset_generator, ['images', 'label'], shuffle=True)
    ...
    config_ckpt = CheckpointConfig(save_checkpoint_steps=32, keep_checkpoint_max=15)
    #prefix表示生成CheckPoint文件的前缀名；directory：表示存放模型的目录
    cbs_1 = ModelCheckpoint(prefix='deeplabv3', directory='/cache/output_train', config=config_ckpt)
    ...
    #关联训练完成后的模型路径
    mox.file.copy_parallel(src_url='/cache/output_train', dst_url='obs://canncamp-temp/AIAC/output_train')


if __name__ == '__main__':
    # 关联数据路径
    mox.file.copy_parallel(src_url='obs://canncamp-temp/AIAC/gis', dst_url='/cache/gis')
    train()
