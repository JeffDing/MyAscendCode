...
def test_eval(model_ckpt_path):
    ...
    duanyu = input("请输入需要生成的短语：")
    t = input("请输入需要续写还是藏头诗：")
    i = eval(input("请输入需要生成的数量："))
    print("\n")
  
    while (i != 0):
        if (t == "续写"):
            start_time = time.time()
            output = generate_random_poetry(poetrymodel, s=duanyu)
            end_to_end_delay = (time.time()-start_time)*1000
            a = re.findall(r'[\u4e00-\u9fa5]*[\uff0c\u3002]', output)
            ...
            start_time = time.time()
            output = generate_hidden(poetrymodel, head=duanyu)
            end_to_end_delay = (time.time()-start_time)*1000
            a = re.findall(r'[\u4e00-\u9fa5]*[\uff0c\u3002]', output)
            print("\n**********************************")
            print("藏头诗 【{}】: \n".format(duanyu))
