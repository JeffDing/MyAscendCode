...
class ConvAE(object):
    def __init__(self, n_input, kernel_size,n_hidden, num_modalities=2,learning_rate = 1e-3, batch_size = 256,\
        ...
        self.optimizer = tf.train.AdamOptimizer(learning_rate = learning_rate)#GradientDescentOptimizer #AdamOptimizer
        
        """
        loss_scale code
        """
        loss_scale_manager = ExponentialUpdateLossScaleManager(init_loss_scale=2**32, incr_every_n_steps=1000, decr_every_n_nan_or_inf=2, decr_ratio=0.5)
        loss_scale_optimizer = NPULossScaleOptimizer(self.optimizer, loss_scale_manager)
        self.optimizer=loss_scale_optimizer.minimize(self.loss)
        ...
        tfconfig.graph_options.rewrite_options.remapping = RewriterConfig.OFF  # 必须显式关闭
        tfconfig.graph_options.rewrite_options.memory_optimization = RewriterConfig.OFF  # 必须显式关闭
...
def train_face(Img, CAE, n_input, batch_size,num_modalities,max_epochs):
    ...
    while it<max_epochs:
        ...
        epoch_time = time.time()-start_time_per_epoch
        if it % display_step == 0:            
            print("epoch: %.1d" % _epochs)
            print("cost: %.8f" % avg_cost)
            print("step time: %.4f" % epoch_time)
        if it % save_step == 0:
            CAE.save_model()
    return
...
if __name__ == '__main__':
    ...
    num_modalities1 = int(data['num_modalities'])
