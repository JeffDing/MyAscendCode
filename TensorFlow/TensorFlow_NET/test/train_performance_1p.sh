...
if [ x"${modelarts_flag}" != x ];
then
    python3.7 ./LeNet.py --data_path=${data_path} --output_path=${output_path} --steps=${train_steps} --epochs=${train_epochs} --batch_size=${batch_size}
else
    python3.7 ./LeNet.py --data_path=${data_path} --output_path=${output_path} --steps=${train_steps} --epochs=${train_epochs} --batch_size=${batch_size} 1>${print_log} 2>&1
fi
...
# 精度相关数据计算
train_accuracy=`grep "accuracy :" ${print_log}  | awk '{print $NF}'`
# 提取所有loss打印信息
grep "loss:" ${print_log} | awk '{print $6}' > ${output_path}/my_output_loss.txt
