...
def main(_):
  ...
  #custom_op.parameter_map["profiling_options"].s = tf.compat.as_bytes('{"output":"./profiling/","task_trace":"on","aicpu":"on"}')
  # dump_path：dump数据存放路径，该参数指定的目录需要在启动训练的环境上（容器或Host侧）提前创建且确保安装时配置的运行用户具有读写权限
  #custom_op.parameter_map["dump_path"].s = tf.compat.as_bytes("./dump/output") 
  ...
  with sess.as_default():
    ...
    #        learning_rate_input).minimize(cross_entropy_mean)
      ...
      for training_step in xrange(start_step, training_steps_max + 1):
        ...
        start = time.time() # new add
        tf.logging.info('Step #%d: rate %f, accuracy %.2f%%, cross entropy %f, %f seconds/step' %
                        (training_step, learning_rate_value, train_accuracy * 100,
                         cross_entropy_value, avg_time_per_step))
