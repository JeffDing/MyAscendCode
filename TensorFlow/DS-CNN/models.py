...
def create_conv_model(fingerprint_input, model_settings, is_training):
  ...
  if is_training:
    first_dropout = npu_ops.dropout(first_relu, dropout_prob)
  else:
  ...
  if is_training:
    second_dropout = npu_ops.dropout(second_relu, dropout_prob)
  else:
...
def create_low_latency_conv_model(fingerprint_input, model_settings,
  ...
  if is_training:
    second_fc_input = npu_ops.dropout(first_fc, dropout_prob)
  else:
  ...
  if is_training:
    final_fc_input = npu_ops.dropout(second_fc, dropout_prob)
  else:
...
def create_dnn_model(fingerprint_input, model_settings, model_size_info, 
  ...
  for i in range(1, num_layers + 1):
      with tf.variable_scope('fc'+str(i)):
          ...
          if is_training:
            flow = npu_ops.dropout(flow, dropout_prob)
...
def create_cnn_model(fingerprint_input, model_settings, model_size_info,
  ...
  if is_training:
    final_fc_input = npu_ops.dropout(first_fc, dropout_prob)
  else:
