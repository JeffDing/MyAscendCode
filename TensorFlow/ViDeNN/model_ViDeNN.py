...
#
		...
		for idx in tqdm(range(len(eval_files)-1)):
			if idx==0:
				...
				ssim = cal_ssim(test,temp_clean_image[0,0])
				ssim1 = cal_ssim(test,out[0,0])
				psnr_sum += psnr
				ssim_sum += ssim  
				...
				if print_psnr: 
					...
					print(" frame %d denoised, SSIM: %.2f" % (idx, ssim))
			...
			if idx==0:
				...
				ssim = cal_ssim(test1,out[0,1])
				ssim1 = cal_ssim(test1, temp_clean_image[0,0])
			else:
				psnr1 = psnr_scaled(test2,previous)
				psnr = psnr_scaled(test2, temp_clean_image[0,0])
				ssim = cal_ssim(test2, temp_clean_image[0,0])
				ssim1 = cal_ssim(test2,previous)
				try:
					test3
				except NameError:
					test3=test2
			...
			if print_psnr: 
				...
				print(" frame %d denoised, SSIM: %.2f" % (idx+1, ssim))
		...
		avg_ssim = ssim_sum / len(eval_files)
		if print_psnr: 
